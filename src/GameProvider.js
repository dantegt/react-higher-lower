...
export default function GameProvider({ children }) {
    ...
    const onPlayAgain = () => {
        const newDeck = shuffleDeck([...sourceDeck])
        setChecking(false)
        setDeck(newDeck)
        setScore(0)
        setTurn(1)
    ...
    const onGuess = (guess) => {
        ...
        const isJoker = currentCard.value === "JOKER" || newCard.value === "JOKER"
    ...
    return (
        <GameDataContext.Provider value={{ play, deck, score, guesses, turn, checking, gameOver, currentCard, nextCard, onDrawCard, onStartPlaying, onGuess, onPlayAgain }}>
            { children }
...
function shuffleDeck (deck) {
    return deck.map((card) => ({sort: Math.random(), value: card}))
